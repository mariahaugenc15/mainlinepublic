...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    await sql(`CREATE TABLE IF NOT EXISTS company_settings (id TEXT PRIMARY KEY DEFAULT 'singleton', company_name TEXT NOT NULL DEFAULT 'Your Company', address TEXT NOT NULL DEFAULT '', phone TEXT NOT NULL DEFAULT '', support_email TEXT NOT NULL DEFAULT '', website TEXT NOT NULL DEFAULT '', logo_path TEXT NOT NULL DEFAULT '', trade_license TEXT NOT NULL DEFAULT '', insurance_carrier TEXT NOT NULL DEFAULT '', service_area TEXT NOT NULL DEFAULT '', default_markup_pct REAL NOT NULL DEFAULT 20, updated_at TEXT NOT NULL DEFAULT (NOW()::TEXT), billing_plan TEXT NOT NULL DEFAULT 'professional', billing_status TEXT NOT NULL DEFAULT 'trialing', billing_seat_count INTEGER NOT NULL DEFAULT 0, trial_ends_at TEXT, current_period_start TEXT, current_period_end TEXT, stripe_customer_id TEXT, stripe_subscription_id TEXT)`);
    await sql(`INSERT INTO company_settings (id, default_markup_pct) VALUES ('singleton', 20) ON CONFLICT (id) DO NOTHING`);
    await sql(`CREATE TABLE IF NOT EXISTS billing_events (id TEXT PRIMARY KEY, description TEXT NOT NULL, amount REAL NOT NULL, status TEXT NOT NULL DEFAULT 'paid', stripe_invoice_id TEXT, created_at TEXT NOT NULL DEFAULT (NOW()::TEXT))`);
